...
class Constants:
    def __init__(self):
        ...
        self.COLOR_FORMULA_EXIST="#DDEDF7"
...
class Variables:
    ...
    formulas = []
    gridCellCurrentValue = []                                                 #store value of current cell selected
...
def cellSelected(event,variableObj):
    ...
    currentCellRow=event.GetRow()
    currentCellCol=event.GetCol()
    variableObj.gridCellCurrentValue.insert(0,grid.GetCellValue(currentCellRow,currentCellCol))
...
def cellValueChanged(event,variableObj):
    ...
    if(((currentCellCol!=0) and (currentCellRow!=0)) == False):            #for symbol and module row
...
#######################################################################################################################################################
    ...
    #cell is the cellAddress in formula list
        ...
        if(grid.GetCellValue(currentCellRow,currentCellCol) == ""):                     # if the cell is emptied by the user and the cell is being used by the other cell for formula
            grid.SetCellValue(currentCellRow,currentCellCol,"0")                        #changing empty cell with value 0. So that all cell containing this cell as formula entity can change to 0
            valueRecalculate(grid,gridIndex,variableObj,currentCellAddress)
            grid.SetCellValue(currentCellRow,currentCellCol,"")

        else:
            if(grid.GetCellValue(currentCellRow,currentCellCol)[0] == "="):             #if cell is a formula
                formulaParser(variableObj,grid,gridIndex,currentCellRow,currentCellCol,grid.GetCellValue(currentCellRow,currentCellCol))
            else:
                if(grid.GetCellValue(currentCellRow,currentCellCol)[0] == "#"):          #if cell is a text
                    formulaParser(variableObj,grid,gridIndex,currentCellRow,currentCellCol,grid.GetCellValue(currentCellRow,currentCellCol))
                else:
                    try:                                                                        # to check if the input address is hex type
                        int(grid.GetCellValue(currentCellRow,currentCellCol),16)
                        valueRecalculate(grid,gridIndex,variableObj,currentCellAddress)
                    except:
                        grid.SetCellValue(currentCellRow,currentCellCol,variableObj.gridCellCurrentValue[1])




def valueRecalculate(grid,gridIndex,variableObj,currentCellAddress):
